...
function CAOL(x::SignalBank{N},λ::Real,h0::SignalBank{M};
        p=0,maxiters=2000,tol=1e-13,trace=false) where {N,M}
    @assert N == M "Signal dimension must match filter dimension"
    @assert p < length(h0)
    ...
    if !trace
        ...
        return [h0[1:p]; Array.(_filterlist(H,R))], 
            [_filterlist(Ht,R) for Ht in Htrace], objtrace, Hdifftrace
    end
end

# x::SignalBank, (H0,R)::SignalTuple
function CAOL(x::SignalBank{N},λ::Real,(H0,R)::SignalTuple{M};
        p=0,maxiters=2000,tol=1e-13,trace=false) where {N, M}
    @assert N == M "Signal dimension must match filter dimension"
    @assert p < size(H0,2)
